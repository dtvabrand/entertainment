    ...
    for site in sorted(sc.keys()):
        ...
        if entries:
            ...
            for disp,tag,xml in entries:
                ...
                links=[]
                if blob_base:
                    if lm: links.append(f'<span style="font-size:75%">[<a href="{blob_base}/m_playlist.m3u8#L{lm}">M</a>]</span>')
                    if ld: links.append(f'<span style="font-size:75%">[<a href="{blob_base}/d_playlist.m3u8#L{ld}">D</a>]</span>')
